...
public class DownloadThreadPool {
    ...
    // ThreadPoolExecutor 是 ExecutorService 的 子类
    ...
    public void execute(DownloadTask task) {
        if (task.isRunning()) { // 如果这个线程在跑，就直接返回
            return;
        }
        cancel(task);  // 先取消这个线程，防止线程重复
        synchronized (this) {
            //  private Map<String, DownloadTask> mRunnableHolder = new HashMap<>();
            // 将下载线程缓存起来，用task.tag() 区分 tag 其实就是url。
            mRunnableHolder.put(task.tag(), task);
        }
        mRunnablePool.execute(task); // 执行这个线程
    }
