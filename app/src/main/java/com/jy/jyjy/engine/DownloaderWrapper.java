...
public final class DownloaderWrapper {
    private static List<VideoInfo> sDLVideoList = new ArrayList<>(); // 下载的列表数据
    private static RxBus sRxBus; // RxBus 用户更新界面
    private static VideoInfoDao sDbDao; // 用户缓存下载列表，以及下载进度等等。
    ...
    /**
     * 监听器封装类 ，会根据下载的情况进行回调，将下载的情况封装成对象返回。
     */
